...
ltc.peak.demand.county <- ltc.demand.county %>%
  ...
  mutate(year.bins = cut(year,
                         ...
                         labels = c("2025 to 2030", "2030 to 2035", "2035 to 2040", "2040 to 2045", "After 2045"))) %>%
  left_join(mn_counties[,c(5,7)], by = "countyfp")

# Create chart ------------------------------------------------------------

names(ltc.peak.demand.county)

ggplot(ltc.peak.demand.county) +
  geom_sf(color = "grey85", aes(geometry = geometry, fill = year.bins)) +
  scale_fill_manual(values = brewer.pal(5, "PuBu")) +
  theme_sf +
  labs(title= "The year that peak demand for\nskilled nursing beds will peak",
       subtitle = "Counties on the west will experience peak\ndemand significantly sooner than other areas") +
  theme(legend.box.margin = margin(60,0,0,-70),
        text = element_text(size = 16))


ggsave(filename = "Charts/Paper report/skilled-nursing-demand-county.png", dpi = "print", width = 6, height = 5)

ggsave(filename = "Charts/Paper report/skilled-nursing-demand-county.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 5)
