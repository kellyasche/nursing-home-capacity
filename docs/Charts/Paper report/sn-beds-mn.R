...
ggplot(nh.nf2.beds.mn, aes(year, nh.nf2.beds)) +
  ...
  geom_label(data = filter(nh.nf2.beds.mn, year > 2005),aes(y = nh.nf2.beds / 2, label = paste("% change\nsince 2005\n", percent(nh.nf2.beds.pct.change, accuracy = .1), sep = "")), color = "black", size = 3) +
  geom_label(aes(label = comma(nh.nf2.beds)), show.legend = FALSE, color = "black", size = 3, position = position_dodge(width = .9)) +
  labs(x="", y = "", color="", title = "Number of skilled nursing beds", subtitle = "The number of beds has decreased by a third since 2005.")+
...
ggsave(filename = "Charts/Paper report/sn-beds-mn.png", dpi = "print", width = 6, height = 4)

ggsave(filename = "Charts/Paper report/sn-beds-mn.pdf", device = cairo_pdf, dpi = "print", width = 6, height = 4)
