...
* Overall, Minnesota has lost a third of the total skilled nursing facility beds from 2005 to 2024. 
* The regions that experienced the largest decline in skilled nusing facility beds are in Northwest and Southwest Minnesota with a reduction of 43% and 38%. The lowest percentage is actually in Central Minnesota while the twin cities was 30.2% reduction.
* The EDR's with a larger reduction in SNF beds than the state as a whole are;
  * Northwest: EDR 4 and EDR 5
  * Central: EDR 7E
  * Southwest: EDR 8 and EDR 9
* Our entirely rural counties has lost 41.7% of their SNF beds, while it improves incrementally until entirely urban with a 28.9% reduction.
* These reductions do represent trends in that there has been a continuous decline in available SNF beds across Minnesota.

**Availability per 65+ resident**

* In Minnesota, the number of 65+ individuals per SNF licensed bed grew from 20.5 residents per SNF bed to 48.5.
* Surprinsingly, the largest number of 65+ individuals per SNF licensed bed was the seven county metro at 60. But this is driven by four counties - Anoka, Washington, Dakota, and Carver, which all have nearly, or just over 100 65+ per SNF bed. Southwest actually had the lowest number with 28 individuals 65+ per nursing home bed.
* The EDRs with really high ratios include EDR 5 - North Central with 72 individuals 65+ per bed, EDR 7E and 7W with 61 and 65.
* Counties in the central lakes region have extremely high ratios - Cass, Pine, Hubbard, and Crow Wing.
* Surprisingly, our entirely rural counties have the lowest number of individuals 65+ per bed with 23, The highest is in our entirely urban counties.

**Medicare vs. Medicaid beds**

The breakdowns showed that nearly the entire state has 100% of their beds as dual eligible. Roseau County is an outlier, however, for rural Minnesota.
...
Although the metro is experiencing a larger decline in beds, rural Minnesota is experiencing a larger percentage of facilities closing. Northwest has experienced a 22% decline in facilities since 2005. This is followed by Southwest with 20%, Southeast a 17%, Northeast a 14%, Seven County metro at 11% and Central with a 8% decline. If looked at by rural-urban-ness, the more rural a county is the more likely it has experienced higher percentages of facility closures than urban counties. In our entirely rural counties, 26% of their facilities have closed since 2005, compared to 21% in town/rural mix counties, 16% in urban/town/rural mix counties and only 9% in our entirely urban counties.

**Facility expansion, shrinkage or closure**

Overall, a majority of facilities are experiencing shrinkage in their bed capacity. Between 2005 and 2024, 61% have less capacity, 21% of facilities closed, 8.6% of facilities didn't shrink or grow, 6% are new facilities, and 4% expanded their bed capacity.

* Shrinkage or closures: Southwest, Northwest and Northeast are all regions experiencing high percentage of facilities that have closed or are shrinking their bed capacity.
* Closing or no change: Southeast is experiencing a higher percentage of closing facilities and facilities that have changed.
* Shrinking or expanding: Central is experiencing both shrinking in bed capacity among older facilities while also having facilities expanding the beds they offer.
* Closing and opening: The seven county metro has oneo f the lowetst percentages of facilities that are shrinking, but also have a higher percentage that have closed and are brand new.

**Bed declines: shrinkage or closure**

Our entirely rural counties have a significantly higher percentage of their decline in bed capacity due to facility closure rather than shrinkage - 58% closure vs 42% due to shrinkage. The percentages for the other county groups are flipped. 

**Facility size profile**

Facilities in rural areas are significantly smaller. By 2024, the entirely rural county group had an average facility size of 45 beds compared to 59 for town/rural mix, 66 for urban/town/rural mix and 89 for entirely urban areas.

**Size of facilities that closed**

In Minnesota, the average size of facilities that closed was 59 beds at the time of their closing compared to an average size of 72 beds of facilities that are still currently open. The medians were 48 vs. 62. An even larger gap at the 75th quartile with 61 beds vs. 86 for those that are still open. The means and medians were significantly different between the two groups.
...
The chart below shows that these numbers do represent trends. The reduction has been constant and consistent.

<br>

```{r nh and nf2 beds ruca trends, echo = FALSE}
nh.nf2.beds.ruca.trends <- nh.nf2.beds.master %>%
  filter(!is.na(Dem_Desc)) %>%
  group_by(year, Dem_Desc) %>%
  summarize(nh.nf2.beds = sum(nh.nf2.beds)) %>%
  ungroup() %>%
...
nh.nf2.beds.ruca.trends.plot <- ggplot(data = nh.nf2.beds.ruca.trends, aes(year, nh.nf2.beds.pct.change, color = Dem_Desc)) +
  geom_hline(yintercept = 0, color = "black") +
  geom_line(size = 3) +
  geom_point_interactive(position = "dodge", aes(data_id = data_id, tooltip = paste(Dem_Desc, "\n", year,  "\nNursing home beds and Boarding Care Nursing Beds", "\n", year, "\nNumber of beds: ", comma(nh.nf2.beds), "\nChange since ", min(nh.nf2.beds.mn$year), ": ", percent(nh.nf2.beds.pct.change, accuracy = .1), sep = ""))) +
  geom_label_repel(data = filter(nh.nf2.beds.ruca.trends, year == max(year)),aes(label = percent(nh.nf2.beds.pct.change, accuracy = .1)), color = "black", show.legend = FALSE) +
  labs(x="", y = "", color="", title = "Percent change in the number of nursing home beds (nursing and\nboarding homes) since 2005")+
  scale_y_continuous(labels=scales::percent)+
...
girafe(ggobj = nh.nf2.beds.ruca.trends.plot, width_svg = 10, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))      
...
The number of skilled nursing homes has declined signifiacntly in our entirely rural counties compared to the other county groups. 

<br>

```{r facilities ruca, echo=FALSE}
snf.fac.ruca <- snf.fac %>%
  group_by(Dem_Desc, year) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(Dem_Desc) %>%
...
snf.fac.ruca.plot <- ggplot(snf.fac.ruca, aes(year, pct.change, color = Dem_Desc)) +
  geom_line(size = 3) +
  geom_point_interactive(size = 5, aes(data_id = data_id, tooltip = paste(Dem_Desc, "\nYear: ", year, "\nNumber of facilities: ", comma(n), "\nPercent change since 2005: ", percent(pct.change, accuracy = .1), sep = ""))) +
  labs(x="", y = "", color="", title = "Number of facilities with skilled nursing beds", subtitle = "The largest percentage declines in the number of facilities has\ndecreased more severely in rural Minnesota.")+
  geom_label_repel(data = filter(snf.fac.ruca, year == max(year)), aes(label = percent(pct.change, accuracy = .1)), show.legend = FALSE, size = 5) +
  scale_y_continuous(labels=scales::percent)+
  scale_x_continuous(breaks = seq(1900, 2050, 5)) +
  scale_color_manual(values = color.ruca,
                     guide = guide_legend(ncol = 2)) +
...
girafe(ggobj = snf.fac.ruca.plot, width_svg = 7, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
## Facility closure, expansion, new{.tabset}

Next we want to see if any of the geographic areas are experiencing different trends in terms of new facilities opening, closing, or just shrinking the number of rooms they are offering.

```{r fac groups}
fac.groups.2005  <- snf.fac %>%
  filter(year == min(year)) %>%
  mutate(nh.nf2.beds.2005 = NH_BEDS + NF2_BEDS) %>%
  select(HFID, NAME, nh.nf2.beds.2005, countyfp)

fac.groups.2024 <- snf.fac %>%
  filter(year == max(year)) %>%
  mutate(nh.nf2.beds.2024 = NH_BEDS + NF2_BEDS) %>%
  select(HFID, NAME, nh.nf2.beds.2024, countyfp) 

fac.groups.2005.2024 <- fac.groups.2005 %>%
  full_join(fac.groups.2024, by = c("HFID", "countyfp")) %>%
  mutate(shrunk.closure = ifelse(nh.nf2.beds.2005 > nh.nf2.beds.2024, "Shrink", 99),
         ...
         group = ifelse(shrunk.closure %in% c("Closure", "Shrink"), "Shrinking or closed", "Expanding, new facilities, no change")) %>%
  left_join(counties.regions, by = "countyfp")

head(fac.groups.2005.2024)

names(fac.groups.2005.2024)

```

<br>

Below are charts indicated the percentage of facilities that fall into the following categories with sub-groups;

* Shrinking or closing: facilities that are either;
  + shrinking - offer less beds in 2024 than in 2005, or
  + closing - the facilities has closed since 2005.
* Expanding, new facilities, or no change: facilities that are either;
  + expanding - offer more beds in 2024 than in 2005,
  + new facilities - the facility did not exist in 2005, or
  + no change - offers the same number of beds in 2024 as in 2005.

<br>

### Minnesota

Nearly 61% of the skilled nursing facilities in 2024 has shrunk the number of beds they had available, followed by 21% of the facilities that have closed. 

<br>

```{r fac groups mn, echo=FALSE}
fac.groups.mn <- fac.groups.2005.2024 %>%
  group_by(group, shrunk.closure) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  mutate(shrunk.closure.pct = n / sum(n)) %>%
  group_by(group) %>%
  mutate(n.group = sum(n)) %>%
  ungroup() %>%
  mutate(group.pct = n.group / sum(n),
         data_id = seq(n()),
         group = fct_relevel(group, "Shrinking or closed", "Expanding, new facilities, no change")) %>%
  arrange(desc(group)) %>%
  group_by(group) %>%
  mutate(shrunk.closure.rolling.sum = cumsum(shrunk.closure.pct)) %>%
  ungroup()

fac.groups.mn.plot <- ggplot(fac.groups.mn, aes(group, shrunk.closure.pct, fill = shrunk.closure, group = shrunk.closure)) +
  geom_col_interactive(position = "stack", aes(data_id = data_id, tooltip = paste("Minnesota", "\n2024", "\nFacility group change sine 2005: ", group, "\nType of change: ", shrunk.closure, "\nNumber of facilities changed since 2005: ", comma(n), "\nPercentage of facilities changed since 2005: ", percent(group.pct, accuracy = .1), sep = ""))) +
  geom_label(aes(label = percent(shrunk.closure.pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5, position = position_stack(vjust = .5)) +
  labs(x="", y = "", color="", title = "Percent of skilled nursing facilities by change in beds")+
  geom_label(aes(y = group.pct, label = percent(group.pct, accuracy = .1)), show.legend = FALSE, color = "black", fill = "white") +
  scale_y_continuous(labels=scales::percent)+
  scale_fill_manual(values = brewer.pal(n = 6, "RdYlBu")) +
  theme_bar+
  theme(legend.position = "bottom",
...
girafe(ggobj = fac.groups.mn.plot, width_svg = 7, height_svg = 7) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))      
...
The chart below shows that the our most rural areas of Minnesota have the highest percentage of facilities that are either shrinking the number of beds they have or are closing. Northwest, Northeast, and Southwest each have percentages near or over 90%. 

Central, Seven county metro, and southeast have highest percentages of facilities that are either expanding or has no change. 



<br>

```{r fac groups pr, echo=FALSE}
fac.groups.pr <- fac.groups.2005.2024 %>%
  group_by(planning.region, group, shrunk.closure) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(planning.region) %>%
  mutate(shrunk.closure.pct = n / sum(n)) %>%
...
fac.groups.pr.plot <- ggplot(fac.groups.pr, aes(planning.region, shrunk.closure.pct, fill = shrunk.closure, group = shrunk.closure)) +
...
girafe(ggobj = fac.groups.pr.plot, width_svg = 8, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))      
...
```{r fac groups edr, echo=FALSE}
fac.groups.edr <- fac.groups.2005.2024 %>%
  group_by(edr, planning.region, group, shrunk.closure) %>%
  summarize(n = n()) %>%
  ungroup() %>%
  group_by(edr, planning.region) %>%
  mutate(shrunk.closure.pct = n / sum(n)) %>%
  ungroup() %>%
  group_by(edr, planning.region, group) %>%
  mutate(n.group = sum(n)) %>%
  ungroup() %>%
  group_by(edr, planning.region) %>%
  mutate(group.pct = n.group / sum(n)) %>%
...
fac.groups.edr.plot <- ggplot(fac.groups.edr, aes(edr, shrunk.closure.pct, fill = shrunk.closure, group = shrunk.closure, color = planning.region)) +
  facet_wrap(~group, ncol = 1, scales = "free_y") +
  geom_col_interactive(position = "stack", aes(data_id = data_id, tooltip = paste(edr, "\n2024", "\nFacility group change sine 2005: ", group, "\nType of change: ", shrunk.closure, "\nNumber of facilities changed since 2005: ", comma(n), "\nPercentage of facilities changed since 2005: ", percent(group.pct, accuracy = .1), sep = ""))) +
  geom_label(aes(label = percent(shrunk.closure.pct, accuracy = .1)), show.legend = FALSE, color = "black", size = 5, position = position_stack(vjust = .5)) +
  labs(x="", y = "", color="", title = "Percent of skilled nursing facilities by change in beds")+
  geom_label(aes(y = group.pct, label = percent(group.pct, accuracy = .1)), show.legend = FALSE, color = "black", fill = "white") +
  scale_y_continuous(labels=scales::percent)+
...
girafe(ggobj = fac.groups.edr.plot, width_svg = 8, height_svg = 14) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))      
...
```{r fac groups ruca, echo=FALSE}
fac.groups.ruca <- fac.groups.2005.2024 %>%
  ...
  ungroup() %>%
  mutate(data_id = seq(n()),
         shrunk.closure = fct_reorder(shrunk.closure, -n)) 

fac.groups.ruca.plot <- ggplot(fac.groups.ruca, aes(shrunk.closure, pct, fill = Dem_Desc, group = Dem_Desc)) +
  geom_col_interactive(position = "dodge", aes(data_id = data_id, tooltip = paste(Dem_Desc, "\n2024", "\nFacility group change sine 2005: ", shrunk.closure, "\nNumber of facilities changed since 2005: ", comma(n), "\nPercentage of facilities changed since 2005: ", percent(pct, accuracy = .1), sep = ""))) +
...
girafe(ggobj = fac.groups.ruca.plot, width_svg = 8, height_svg = 7) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_sizing(rescale = FALSE))      
...
Instead of breaking it up like this, I think it might be easier to summarize if I split facilities into two groups;

1. Shrinking or closing: Facilities that have either reduced
...
* Expansion: Beds in 2005 were less than beds in 2024.
* New facility: Beds in 2005 were NA and values in 2024 were greater than 0.
...
head(fac.nh.nf2.beds.2005.2024)

names(fac.nh.nf2.beds.2005.2024)
```

<br>

The new object, "fac.nh.nf2.beds.2005.2024" has `r comma(nrow(fac.nh.nf2.beds.2005.2024))` rows.

The charts below provide the percentage of reduction in beds that were due to facility closures and facility shrinkage. At a larger geographic level, bed reductions due to facility shrinkage makes up over 50% of the bed reductions. However, as we drill deeper, more rural counties and geographic regions experience a higher degree of bed reductions due to facility closures than facility shrinkages.

In our entirely rural counties, 58% of the reduction in beds is due to facility closures compared to 42% of facility shrinkage.
...
## Facility size profile{.tabset}

Let's first do a quick profile of the regions in Minnesota. What are the typical sizes of nursing home facilities across rural areas. First we will compare the composition of facilities in 2005 vs. 2024.

The charts below provide each facility (black dot) along with their bed capacity (location on y axis). It consistently shows that more urban areas have larger facilities compared to rural regions. In addition, the statistics decline across all regions and rural-urban county groups from 2005 to 2024.

<br>

```{r nursing home sizes, echo=FALSE}
fac.size.county <- nh.nf2.beds.master %>%
...
```{r fac size MN, echo=FALSE}
fac.size.summary.mn <- fac.size.county %>%
  group_by(year) %>%
  summarize(quantile1 = quantile(nh.nf2.beds, .25),
            mean = mean(nh.nf2.beds),
            median = median(nh.nf2.beds),
            quantile3 = quantile(nh.nf2.beds, .75)) %>%
  ungroup()

fac.size.mn.plot <- ggplot(fac.size.county, aes(as.factor(year), nh.nf2.beds)) +
  geom_hline(yintercept = 0, color = "black") +
  geom_boxplot(outlier.color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(year, "\n", NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  geom_text(data = fac.size.summary.mn, aes(x = as.factor(year), y = max(fac.size.county$nh.nf2.beds) - 50, label = paste("Quantile 25: ", comma(quantile1), "\nMean: ", comma(mean), "\nMedian: ", comma(median), "\nQuantile 75: ", comma(quantile3), sep = "")), color = "#006d2c") +
  labs(x="", y = "", color="", title = "Facility bed capacity in Minnesota")+
  theme_line+
...
girafe(ggobj = fac.size.mn.plot, width_svg = 7, height_svg = 7) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
The more rural a planning region is the smaller the facilities - Northwest, and Southwest have the smallest facilities while the seven county metro has significantly larger facilities. But all statistics decline across all planning regions from 2005 to 2024.

<br>

```{r fac size pr, echo=FALSE}
fac.size.summary.pr <- fac.size.county %>%
  group_by(year, planning.region) %>%
  summarize(quantile1 = quantile(nh.nf2.beds, .25),
            mean = mean(nh.nf2.beds),
            median = median(nh.nf2.beds),
...
fac.size.pr.plot <- ggplot(fac.size.county, aes(planning.region, nh.nf2.beds)) +
  facet_wrap(~year, ncol = 1) +
  geom_hline(yintercept = 0, color = "black") +
  geom_boxplot(outlier.color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(year, "\n", NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  geom_text(data = fac.size.summary.pr, aes(x = planning.region, y = max(fac.size.county$nh.nf2.beds) - 100, label = paste("Quantile 1: ", comma(quantile1), "\nMean: ", comma(mean), "\nMedian: ", comma(median), "\nQuantile 3: ", comma(quantile3), sep = "")), color = "#006d2c") +
  labs(x="", y = "", color="", title = "Facility bed capacity in Minnesota")+
  theme_line+
  scale_color_manual(values= brewer.pal(n = 6, "RdYlBu"),
                     guide = guide_legend(ncol = 3)) +
...
girafe(ggobj = fac.size.pr.plot, width_svg = 10, height_svg = 8) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
As the county groups gets more urban, the larger the facilities. In addition, each rural-urban county group has a decline in every statistics from 2005 to 2024.

<br>

```{r fac size ruca, echo=FALSE}
fac.size.summary.ruca <- fac.size.county %>%
  group_by(year, Dem_Desc) %>%
  summarize(quantile1 = quantile(nh.nf2.beds, .25),
            mean = mean(nh.nf2.beds),
            median = median(nh.nf2.beds),
...
fac.size.ruca.plot <- ggplot(fac.size.county, aes(Dem_Desc, nh.nf2.beds)) +
  facet_wrap(~year, ncol = 1) +
  geom_hline(yintercept = 0, color = "black") +
  geom_boxplot(outlier.color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(year, "\n", NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  geom_text(data = fac.size.summary.ruca, aes(x = Dem_Desc, y = max(fac.size.county$nh.nf2.beds) - 100, label = paste("Quantile 1: ", comma(quantile1), "\nMean: ", comma(mean), "\nMedian: ", comma(median), "\nQuantile 3: ", comma(quantile3), sep = "")), color = "#006d2c") +
  labs(x="", y = "", color="", title = "Facility bed capacity in Minnesota")+
  theme_line+
  scale_color_manual(values= brewer.pal(n = 6, "RdYlBu"),
                     guide = guide_legend(ncol = 3)) +
...
girafe(ggobj = fac.size.ruca.plot, width_svg = 8, height_svg = 8) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
## Size of facilities that closed{.tabset}

Now, let's check to see the size of the facilities that have closed since 2005. To do this, we will first create a list of all facilities that closed.

<br>

```{r facilities that have closed}
fac.less5 <- snf.fac %>%
  group_by(HFID) %>%
  summarize(n = n()) %>%
  ungroup()
...
missing.fac %>%
  select(-open.close, -nh.nf2.beds.bins) %>%
  datatable(., class = "cell-border stripe", filter = "top", rownames = FALSE)

```

<br>

Before comparing the bed sizes of facilities that closed to facilities that have remained open, let's check for normal distribution. That way we can use t-tests to check if their means are significantly different.

<br>

```{r fac open close check for normality}
library(ggpubr)
library(rstatix)

snf.fac.open.close %>%
  group_by(open.close) %>%
  shapiro_test(nh.nf2.beds)

```

<br>

Unfortunately, these are not normally distributed. So, we need to use a non-parametric test. For this we will just use Wilcoxon test.
...
The wilcox test results indicate we can reject the NULL hypothesis that the medians are similar.

```{r fac size open and closed mn wilcoxon, echo=FALSE}
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close)
```

<br>


The facilities that have closed since 2005 have significantly lower summary statistics compared to the facilities that are still open. 

<br>

```{r fac size open and closed MN, echo=FALSE}
snf.fac.open.close.mn.summary <- snf.fac.open.close %>%
  group_by(open.close) %>%
  summarize(Q1 = quantile(nh.nf2.beds, .25),
            Mean = mean(nh.nf2.beds),
            Median = median(nh.nf2.beds),
            Q3 = quantile(nh.nf2.beds, .75)) %>%
  ungroup()

fac.size.open.close.mn.plot <- ggplot(snf.fac.open.close, aes(x = open.close, y = nh.nf2.beds)) +
  geom_hline(yintercept = 0, color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  labs(x="", y = "", color="", title = "Facility that closed since 2005 in Minnesota")+
  geom_text(data = snf.fac.open.close.mn.summary, aes(x = open.close, y = max(snf.fac.open.close$nh.nf2.beds) - 50, label = paste("Quantile 25: ", comma(Q1), "\nMean: ", comma(Mean), "\nMedian: ", comma(Median), "\nQuantile 75: ", comma(Q3), sep = "")), color = "#006d2c") +
  theme_line+
  theme(legend.position = "bottom",
        text = element_text(size = 18))

girafe(ggobj = fac.size.open.close.mn.plot, width_svg = 7, height_svg = 7) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
The p-values below show a mixed bag. A few planning regions fail to reject the NULL hypothesis; Northeast, Central, and Southeast. 

```{r fac size open and closed pr wilcoxon, echo=FALSE}
snf.fac.open.close.nw <- snf.fac.open.close %>%
  filter(planning.region == "Northwest")

snf.fac.open.close.ne <- snf.fac.open.close %>%
  filter(planning.region == "Northeast")

snf.fac.open.close.c <- snf.fac.open.close %>%
  filter(planning.region == "Central")

snf.fac.open.close.tc <- snf.fac.open.close %>%
  filter(planning.region == "Seven County Mpls-St Paul")

snf.fac.open.close.sw <- snf.fac.open.close %>%
  filter(planning.region == "Southwest")

snf.fac.open.close.se <- snf.fac.open.close %>%
  filter(planning.region == "Southeast")

wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.nw)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.ne)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.c)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.tc)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.sw)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.se)
...
The summary statistics of facilities that closed are significantly lower than the ones that are still open across all regions. The most significant is in the twin cities, but that is largely due to the fact that they have very large facilities there. If anything, it provides a clear picture that for the most part 

<br>

```{r fac size open and closed pr, echo=FALSE}
snf.fac.open.close.pr.summary <- snf.fac.open.close %>%
  group_by(planning.region, open.close) %>%
  summarize(Q1 = quantile(nh.nf2.beds, .25),
            Mean = mean(nh.nf2.beds),
            Median = median(nh.nf2.beds),
            Q3 = quantile(nh.nf2.beds, .75)) %>%
...
fac.size.open.close.pr.plot <- ggplot(snf.fac.open.close, aes(x = open.close, y = nh.nf2.beds)) +
  facet_wrap(~planning.region, ncol = 2) +
  geom_hline(yintercept = 0, color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  labs(x="", y = "", color="", title = "Facility that closed since 2005 by planning region")+
  geom_text(data = snf.fac.open.close.pr.summary, aes(x = open.close, y = max(snf.fac.open.close$nh.nf2.beds) - 100, label = paste("Quantile 25: ", comma(Q1), "\nMean: ", comma(Mean), "\nMedian: ", comma(Median), "\nQuantile 75: ", comma(Q3), sep = "")), color = "#006d2c") +
  theme_line+
  theme(legend.position = "bottom",
        text = element_text(size = 18))

girafe(ggobj = fac.size.open.close.pr.plot, width_svg = 7, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
The p-values below show a mixed bag. The entirely rural county group fails to reject the null hypothesis while the others do reject the null. 

```{r fac size open and closed ruca wilcoxon, echo=FALSE}
snf.fac.open.close.er <- snf.fac.open.close %>%
  filter(Dem_Desc == "Entirely rural")

snf.fac.open.close.tr <- snf.fac.open.close %>%
  filter(Dem_Desc == "Town/rural mix")

snf.fac.open.close.utr <- snf.fac.open.close %>%
  filter(Dem_Desc == "Urban/town/rural mix")

snf.fac.open.close.eu <- snf.fac.open.close %>%
  filter(Dem_Desc == "Entirely urban")

wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.er)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.tr)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.utr)
wilcox.test(nh.nf2.beds ~ open.close, data = snf.fac.open.close.eu)
...
The charts below indicate that yes, the entirely rural county group has very similar facility sizes that are open and have closed. This is due to their being no facilities much larger than 100. As the county groups gets more urban, the larger the facilities. In addition, each rural-urban county group has a decline in every statistics from 2005 to 2024.

<br>

```{r fac size open and closed ruca, echo=FALSE}
snf.fac.open.close.ruca.summary <- snf.fac.open.close %>%
  group_by(Dem_Desc, open.close) %>%
  summarize(Q1 = quantile(nh.nf2.beds, .25),
            Mean = mean(nh.nf2.beds),
            Median = median(nh.nf2.beds),
            Q3 = quantile(nh.nf2.beds, .75)) %>%
...
fac.size.open.close.ruca.plot <- ggplot(snf.fac.open.close, aes(x = open.close, y = nh.nf2.beds)) +
  facet_wrap(~Dem_Desc, ncol = 2) +
  geom_hline(yintercept = 0, color = "black") +
  geom_point_interactive(aes(data_id = data_id, tooltip = paste(NAME, "\nBed capacity: ", comma(nh.nf2.beds), sep = ""))) +
  labs(x="", y = "", color="", title = "Facility that closed since 2005 by rural-urban county groups")+
  geom_text(data = snf.fac.open.close.ruca.summary, aes(x = open.close, y = max(snf.fac.open.close$nh.nf2.beds) - 100, label = paste("Quantile 25: ", comma(Q1), "\nMean: ", comma(Mean), "\nMedian: ", comma(Median), "\nQuantile 75: ", comma(Q3), sep = "")), color = "#006d2c") +
  theme_line+
  theme(legend.position = "bottom",
        text = element_text(size = 18))

girafe(ggobj = fac.size.open.close.ruca.plot, width_svg = 7, height_svg = 10) %>%
  girafe_options(opts_selection(type = "none"),
                 opts_toolbar(saveaspng = FALSE),
                 opts_sizing(rescale = FALSE))
...
Overall, it's safe to say that smaller facilities are closing.