...
nh.nf2.beds.age.65plus.county <- nh.nf2.beds.master %>%
  ...
  ungroup() %>%
  filter(year > 2005) %>%
  left_join(master.age.65plus.county, by = c("countyfp", "year")) 
```
...
```{r 65 plus per nh nf2 bed county}
...
nh.nf2.beds.age.65plus.county.map.plot <- ggplot(data = filter(nh.nf2.beds.age.65plus.county.map, year == max(year))) +
